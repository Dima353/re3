...
#ifdef LIBRW
#ifdef RW_OPENGL
	// TODO: other GL targets still take the conversion path
	return false;
#else
	return true;
#endif
...
{
#ifdef PSP2
	// Loose TXDs outside gta3.img are still in their original platform format, and
	// librw-vita cannot read those at load time. Rewrite them in place once, on the
	// same pass that builds the video-card image.
